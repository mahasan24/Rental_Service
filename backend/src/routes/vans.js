...
router.get('/', async (req, res, next) => {
  try {
    ...
    const params = [];
    let i = 1;
    if (type) { query += ` AND type ILIKE $${i}`; params.push(`%${type}%`); i++; }
    if (minCapacity) { query += ` AND capacity >= $${i}`; params.push(parseInt(minCapacity, 10)); i++; }
    if (maxPrice) { query += ` AND price_per_day <= $${i}`; params.push(parseFloat(maxPrice)); i++; }
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    if (result.rows.length === 0) return res.status(404).json({ error: 'Van not found' });
